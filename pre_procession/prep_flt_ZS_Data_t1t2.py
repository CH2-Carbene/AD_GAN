...
def make_one_flt(pname,pdict,pi,outputs):
    ...
    TMP=f"flt_t2/{pi}"
    OUT=f"paired_t2/{pi}"
    PPDIR=glob.glob(f"/public_bme/data/gujch/ZS_t1_proc/{pi[5:]}/*/mr_brain/")[0]
    DNDIR=glob.glob(f"/public_bme/data/gujch/ZS_t1/{pi[5:]}/*/")[0]
    ...
    sh(f"cp -t {TMP} {T2DIR}/* {PPDIR}/intensity-tissue_hemisphere.nii.gz {PPDIR}/intensity.nii.gz {DNDIR}/t1_denoise.nii.gz",name="0_getfile",base_dir=".")

    sh(f"N4BiasFieldCorrection -d 3 --input-image t1_denoise.nii.gz --output t1_ori.nii.gz",name="t1_N4correction")
    ...
    sh(f"flirt -in t1_ori.nii.gz -ref intensity -out t1_acpc -omat t1_ACPC.mat -bins 256 -cost normmi -interp trilinear -searchrx -90 90 -searchry -90 90 -searchrz -90 90 -dof 9",name="3_flirt_ACPC")
    ...
    sh(f"flirt -in t2_ori -ref t1_acpc -out t2_acpc -applyxfm -init t2_ACPC.mat -interp trilinear",name="5_apply_flirt")
    sh(f"fslmaths t1_acpc -mas intensity-tissue_hemisphere T1")
    sh(f"fslmaths t2_acpc -mas intensity-tissue_hemisphere T2")
