...
def make_one_mask(sub,outputs):
    ...
    if os.path.exists(f"{OUTDIR}/{sub}/fnlt.nii.gzWarped.nii.gz"):
        return
    # sh(rf'printf "processing $sub...\n"')
    fullsub=f"{INDIR}/{sub}"
    outsub=f"{OUTDIR}/{sub}"
    os.makedirs(outsub,exist_ok=True)
    sh(f"antsRegistrationSyN.sh -d 3 -f {TEM} -m {fullsub}/T1.nii.gz -o {outsub}/fnlt.nii.gz")
    # sh(f'mkdir -p {outsub}')
