...
def make_one_T1(pname,pdict,outputs):
    ...
    if t1fp.endswith(".tar.gz"):
        ...
        t1fp=t1fp[:-4]
        sh(f"dcm2niix -b y -z y -x n -t n -m n -f t1 -o . -s n -v n {t1fp}",name="1_dcm2nii_t1")
    elif t1fp.endswith(".mgz"):
        sh(f"mri_convert ../{t1fp} t1.nii.gz")
    elif t1fp.endswith(".nii.gz"):
        sh(f"cp ../{t1fp} t1.nii.gz")
    else:
        sh(f"dcm2niix -b y -z y -x n -t n -m n -f t1 -o . -s n -v n ../{t1fp}",name="1_dcm2nii_t1")
    ...
    sh(f"/public_bme/data/gujch/sh/ROBEX/runROBEX.sh t1_n4correct.nii.gz t1_ori_brain.nii.gz",name="1_bet_T1")
    ...
    # sh(f"flirt -in t1_n4correct.nii.gz -ref ~/data/template/MNI152_T1_0.8mm.nii.gz -out t1_ACPC.nii.gz -omat t1_ACPC.mat -dof 7",name="5_t1_to_acpc")
    
    # sh(f"bet t1_ACPC.nii.gz t1_ACPC_brain.nii.gz -f 0.50 -R -s",name="6_t1_bet")

    os.makedirs(f"result/{pname}",exist_ok=True)
    sh(f"cp -t result/{pname} {TMP}/T1.nii.gz","getresult",base_dir="..")
