...
def make_one_flt(pname,pdict,pi,outputs):
    ...
    sh(f"flirt -in t1_ori -ref ~/template/MNI152_T1_0.8mm.nii.gz -out T1 -omat b0_ACPC1.mat -bins 256 -cost corratio -searchrx -90 90 -searchry -90 90 -searchrz -90 90 -dof 6",name="1_flirt_ACPC")
    sh(f"flirt -in b0 -ref t1_ori -omat b0_ACPC2.mat -bins 256 -cost corratio -searchrx -90 90 -searchry -90 90 -searchrz -90 90 -dof 6",name="2_flirt_T1")
...
if __name__=='__main__':
    ...
    for i,pid in enumerate(sorted(ptdict.keys())):
        if str(i) in finished_set:continue
        ptdir=ptdict[pid]
        t1dir=ptdir.get("t1")
