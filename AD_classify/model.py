...
def get_train_ds(train):
    ...
    train_dataset = train_dataset.map(map_func=train_load,num_parallel_calls=20)
...
def get_test_ds(test):
    ...
    # print(train_dataset)
    # train_dataset=load_image_train(train)
    test_dataset = test_dataset.map(map_func=test_load,num_parallel_calls=20)
    # train_dataset = train_dataset.shuffle(BUFFER_SIZE,seed=114514)
    return test_dataset
...
if __name__ == '__main__':
    ZSPATH="/hpc/data/home/bme/v-gujch/work/AD_GAN/datasets/Zhongshan_prep/npdata_2mm"
    # ZSPATH=r"C:\Users\CH2\Documents\datasets\Zhongshan_prep\npdata_clf"
    data=[f"{ZSPATH}/{img}"for img in os.listdir(ZSPATH)]
