...
def get_gen_loss(target, gen_output, disc_fake_output, alpha):
    ...
    L2_loss=get_L2loss(target,gen_output )#tf.math.reduce_mean(tf.math.pow(gen_output - target, 2))
    # disc loss
    disc_loss = tf.math.reduce_mean(tf.math.pow(tf.ones_like(disc_fake_output) - disc_fake_output, 2))

    # total loss
    gen_loss = alpha*L2_loss + disc_loss

    return gen_loss, L2_loss, disc_loss
