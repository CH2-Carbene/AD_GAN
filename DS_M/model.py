...
def Generator_seg(input_shape):
    ...
    for d in range(depth-1, -1, -1):
        if d != 0:
            ...
            x=Conv3D(2, kernel_size=ks0, strides=1,padding='reflect',
                    kernel_initializer="he_normal", activation='softmax')(x)
    outputs=x
    # outputs = ReLU()(x/2+1)-1
    # last = G_conv3d(2, kernel_size=1, strides=2, padding='same', kernel_initializer='he_normal', activation='softmax', name='output_generator')(x)

    return Model(inputs=inputs, outputs=outputs, name='Generator')


def Discriminator_seg(input_shape):
    inputs = Input(input_shape, name='input_image')
    targets = Input(input_shape, name='target_image')
    ks = 4
    depth = 3
...
class DSM:
    def __init__(self, input_shape=(128, 128, 128, 1), alpha=5, example_data=None):
        """
        DSM with paired data. Disc for True and fake image, and L2 loss for voxel loss.
        """
    ...
    def save_checkpoint(self, step, now_loss):
        G1, D1,G2,D2 = self.G1, self.D1,self.G2,self.D2
